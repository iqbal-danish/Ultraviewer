...
pub fn render_csv_grid(
    ...
    let sample_cols: Vec<String> = viewport.lines.first()
        .map(|l| split_delimited(&l.text, delimiter))
        .unwrap_or_default();
    let col_count = sample_cols.len().max(1);

    let available_w = ui.available_width();
    let col_w = (available_w / col_count as f32).clamp(120.0, 300.0);
    ...
    ui.horizontal(|ui| {
        ...
        for i in 0..col_count {
            let col_name = sample_cols.get(i).cloned().unwrap_or_else(|| format!("Col {}", i + 1));
...
fn split_delimited(text: &str, delimiter: char) -> Vec<String> {
