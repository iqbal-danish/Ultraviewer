...
use crate::formats::XmlTreeNode;
use super::icons::{paint_icon, Icon};

pub fn render_xml_tree_panel(
    ui: &mut Ui,
    root: Option<&XmlTreeNode>,
    is_building: bool,
    ...
    if is_building {
        ui.horizontal(|ui| {
            ui.spinner();
            ui.label(RichText::new("Indexing XML structure...").size(12.0).color(Color32::from_rgb(56, 189, 248)));
        });
        ui.add_space(8.0);
    }
    ...
    if let Some(node) = root {
        ...
        ScrollArea::both()
            ...
            .show(ui, |ui| {
                render_tree_node(ui, node, on_jump);
            });
    } else if !is_building {
        ui.add_space(20.0);
        ui.vertical_centered(|ui| {
            ui.label(RichText::new("No XML structure indexed yet.").weak().italics());
        });
    }
}

fn render_tree_node(ui: &mut Ui, node: &XmlTreeNode, on_jump: &mut Option<u64>) {
    let has_children = !node.children.is_empty();

    let header_label = if node.summary.is_empty() {
        node.name.clone()
    } else {
    ...
    if has_children {
        egui::CollapsingHeader::new(RichText::new(&header_label).color(Color32::from_rgb(56, 189, 248)))
            .default_open(true)
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                ...
                for child in &node.children {
                    render_tree_node(ui, child, on_jump);
                }
            });
    } else {
        ui.horizontal(|ui| {
            let resp = ui.selectable_label(false, RichText::new(&header_label).color(Color32::from_rgb(220, 230, 245)));
